...
const User: React.FC = () => {
  ...
  // const [firstNameError, setFirstNameError] = useState<IErrorInput>({ success: true, message: '', });
  // const [lastNameError, setLastNameError] = useState<IErrorInput>({ success: true, message: '', });
  ...
  const handleSaveAllChanges = async () => {
    ...
    try {
      ...
      closeLoading();
      notify(getError(ex)?.message, 'error', 4000);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      setError(`An error occurred while deleting the account. ${getError(ex)?.message}`);
    }
    closeLoading();
  };
